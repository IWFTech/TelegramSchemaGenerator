...
catch (GeneratorConfigurationException exception)
{
    Console.Error.WriteLine("SCHEMA_CONFIGURATION_REQUIRED");
...
static int RunNormalize(GeneratorArguments arguments)
{
    ...
    var configuration = GeneratorConfigurationLoader.Load(arguments.GetRequiredConfigurationPath());
    var normalized = TelegramSchemaNormalizer.Normalize(rawSnapshot, configuration);
